...
import { Menu, X, ChevronDown, Home, BookOpen, Recycle, Zap, BarChart, Leaf } from 'lucide-react';
import { navigationConfig } from '../config';

// Icon lookup map for dynamic icon resolution from config strings
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  Home, BookOpen, Recycle, Zap, BarChart, Leaf, Menu, X, ChevDown: ChevronDown,
};
      ...
      <div className="container-custom flex items-center justify-between">
        ...
        >
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${
            isScrolled ? 'bg-green-500' : 'bg-white/20 backdrop-blur-sm'
          }`}>
            <Leaf className={`w-6 h-6 transition-transform duration-300 group-hover:scale-110 ${
              ...
              >
                ...
                {link.dropdown && (
                  <div
                    ...
                    role="menu"
                  >
                    <div className="bg-white rounded-xl overflow-hidden min-w-[180px] shadow-card-hover border border-gray-100">
                      {link.dropdown.map((item) => (
                        <button
                          key={item.name}
                          onClick={() => scrollToSection(item.href)}
                          className="block w-full text-left px-4 py-3 text-sm text-gray-600 hover:bg-green-50 hover:text-green-600 transition-colors"
                          role="menuitem"
                        >
                          {item.name}
                        </button>
              ...
              >
                {link.dropdown ? (
                    ...
                    >
                      {link.dropdown.map((item) => (
                        <button
                          key={item.name}
                          onClick={() => scrollToSection(item.href)}
                          className="block w-full text-left pl-12 py-3 text-gray-600 hover:text-green-600"
                          role="menuitem"
                        >
                          {item.name}
                        </button>
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 w-full py-4 text-lg text-gray-800 border-b border-gray-100 hover:text-green-600 transition-colors"
                    role="menuitem"
                  >
                    {IconComponent && <IconComponent className="w-5 h-5 text-green-500" />}
                    {link.name}
