...
export function Footer() {
  ...
  const handleNewsletter = async (e: React.FormEvent) => {
    ...
    // Simulate newsletter signup (no actual endpoint)
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <span className="font-display text-xl text-white font-bold block">
                  ...
                  {footerConfig.socialLinks.map((social) => {
                    ...
                    return (
                      <a
                        ...
                        className="w-10 h-10 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center text-gray-400 hover:bg-green-500 hover:border-green-500 hover:text-white transition-all duration-300"
                      >
          ...
          {footerConfig.linkGroups.map((group, index) => (
            <nav key={index} aria-label={group.title}>
              <h3 className="font-display text-lg text-white mb-5 font-semibold">
                {group.title}
              </h3>
      ...
      <div className="border-t border-gray-800">
        <div className="container-custom py-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center justify-center gap-4 text-gray-500 text-xs">
            ...
            {footerConfig.legalLinks.map((link, index) => (
              <span key={index}>
                ...
                <button className="hover:text-green-400 transition-colors ml-2 md:ml-0">
                  {link}
                </button>
              </span>
            ))}
          ...
          {footerConfig.backToTopText && (
            <button
              onClick={scrollToTop}
              className="flex items-center gap-2 text-gray-400 text-sm hover:text-green-400 transition-colors group"
              aria-label={footerConfig.backToTopText}
            >
              <span>{footerConfig.backToTopText}</span>
              <div className="w-8 h-8 rounded-full border border-gray-700 flex items-center justify-center group-hover:border-green-500 group-hover:bg-green-500 transition-all duration-300">
